...
/**
 * Une seule requete pour le departement : compteur exact + villes pour le
 * maillage. La vue publique applique deja le filtre taxi.
 */
...
export async function buildVilleCategorieMetadata(
  ...
  const cat = getCategorieBySlug(categorieSlug);
  if (!cat) return {};
  const pros = await fetchProsVilleCategorie(villeSlug, cat.key);
  const nomVille = pros.length > 0 ? formatNomVille(pros[0].ville) : deslugifyVille(villeSlug);
  ...
  return {
    title,
    description,
    alternates: { canonical: villeCategorieUrl(categorieSlug, villeSlug) },
    openGraph: {
...
export default async function VilleCategorieHub({
  villeSlug,
  categorieSlug,
  ameliOnly,
}: HubProps) {
  const ville = villeSlug;
  const categorie = categorieSlug;
  const cat = getCategorieBySlug(categorie);
  if (!cat) notFound();

  const canonicalPath = villeCategorieUrl(categorie, ville);
  ...
  const jsonLd = {
    ...
    ...buildProsItemList({
      ...
      pros,
      itemUrl: (p) => `${BASE_URL}${ficheUrl(ville, categorie, p.slug)}`,
    }),
  };
  ...
  const regionSlug = departement ? getRegionSlugByDepartement(departement) : null;
  ...
  // Fil d'Ariane hierarchique : Annuaire -> Region -> Departement -> Ville -> Categorie.
  // Les niveaux region et departement renforcent le maillage interne ascendant.
  const breadItems: { name: string; url: string }[] = [
    { name: "Annuaire", url: "/transport-medical" },
  ];
  if (region) {
    breadItems.push({ name: region.nom, url: `/transport-medical/region/${region.slug}` });
  }
  if (depInfo) {
    breadItems.push({
  ...
  breadItems.push(
    ...
    { name: cat.labelPluriel, url: canonicalPath }
  );
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-white via-blue-50/30 to-white">
        ...
        <div className="max-w-6xl mx-auto px-4 py-12">
          <nav className="flex items-center gap-2 text-xs text-blue-200 mb-4 flex-wrap">
            ...
            {region && (
              <>
                <Link
                  href={`/transport-medical/region/${region.slug}`}
          ...
          {/* H1 au singulier sur les hubs a URL courte : "Taxi conventionné à
              Paris" reprend le mot-cle exact recherche, la ou le pluriel
              ("Taxis conventionnés") s'en ecarte. */}
          <h1 className="text-3xl sm:text-4xl font-bold mb-3">
            {utiliseUrlCourte(categorie) ? cat.label : cat.labelPluriel} à {nomVille}
          </h1>
      ...
      <section className="max-w-6xl mx-auto px-4 py-10">
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-2xl p-6 text-gray-700">
            {ameliOnly ? (
              <>
                ...
                <Link href={canonicalPath} className="text-[#0066CC] font-medium hover:underline">
          ...
          <div className="mt-10">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
              {enriched.voisines.map((v) => (
                <Link
                  key={v.slug}
                  href={villeCategorieUrl(categorie, v.slug)}
        ...
        {/* Maillage ascendant : departement ET region, presents sur toutes les
            pages ville x categorie (pas seulement quand la liste est vide). */}
        {(depInfo || region) && (
          <div className="mt-10 flex flex-wrap gap-3 text-sm">
            {depInfo && (
              <Link
                ...
                className="inline-flex items-center gap-1 font-medium text-[#0066CC] hover:underline"
              >
                Transport sanitaire {depInfo.nom} ({depInfo.code})
                <ChevronRight className="w-4 h-4" />
              </Link>
            )}
            {region && (
...
function ProCard({
  ...
  return (
    <Link
      href={ficheUrl(villeSlug, categorieSlug, pro.slug)}
