...
export default async function GarageDashboardPage({
  ...
  // Récupérer les dépôts confiés
  const { data: depots } = await sbService
    .from('depots')
    .select('id, statut, marque, modele, annee, kilometrage, prix_affiche, prix_final_vente, commission_garage_pct, created_at, vendeur_id')
  ...
  const activeTab = (searchParams.tab ?? 'rdv_pris') as TabKey;
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-12">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: "Note moyenne", value: stats.note ? `${Number(stats.note).toFixed(1)} / 5` : '—' },
        ].map((s) => (
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr>
                ...
                <th className="text-left px-5 py-3 text-slate-500 font-medium">Prix affiché</th>
                ...
                <tr key={d.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition">
                  ...
                  <td className="px-5 py-4 text-right">
                    <Link
                      ...
                      className="inline-flex items-center gap-1.5 text-blue-600 hover:text-blue-700 font-medium text-xs"
                    >
